...
export function ModalCloseBoard({board, onArchive}: {board: IBoard, onArchive: (id: string) => void}) {
    ...
    return(
        <div className="group relative">
            <button className={`p-2  ${isBoardPage ? 'hover:bg-white/30' : 'hover:bg-gray-200'}  rounded-md transition-all ease-in-out duration-200 ${isOpen ? 'flex' : 'hidden'} group-hover:flex mr-2 ` }
            onClick={toggleModal}>
                <Ellipsis className='size-4 '/>
            </button>
            {isOpen && (
                <div 
                    className="absolute top-10 left-0 w-72 bg-white shadow-md rounded-md border border-gray-200 flex flex-col p-3 gap-4 text-gray-600 transition-all ease-in-out duration-200 z-50"
                >
